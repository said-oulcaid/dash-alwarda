...
function Details() {
  ...
  const {
    get: getNiveauDetails,
    data: niveauDetails,
    loading: loadingNiveau,
    error: niveauError,
  } = useGet(`niveau/${id}/eleves`);
  ...
  } = useDelete(`eleve`, getNiveauDetails);
  ...
  useEffect(() => {
    filterData();
  }, [niveauDetails, filterEleves]);

  const filterData = () => {
    const { filterByCentre, filterByNom } = filterEleves;
    let filtered = niveauDetails?.Eleves || [];
    if (filterByCentre) {
      filtered = filtered.filter((e) => e.Centre?.id === parseInt(filterByCentre, 10));
    }
    if (filterByNom) {
  ...
  if (loadingNiveau) {
    return (
      <div className="niveau-details-container">
        <div className="niveau-details-content">
          <div className="spinner-grow Loading text-danger" role="status">
            Chargement détails niveau ...
          </div>
        </div>
      </div>
    );
  ...
  if (niveauError) {
    return (
      <div className="niveau-details-container">
        <div className="niveau-details-content">
          <span className="text-danger fs-5">Error: {niveauError?.message}</span>
        </div>
      </div>
    );
  }
    ...
    <div className="niveau-details-container">
      <div className="niveau-details-content">
        <div className="header">
          <h1>Niveau : {niveauDetails?.nom}</h1>
        </div>
        <input
          type="text"
          id="filterByNom"
